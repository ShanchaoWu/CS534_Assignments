...
class HeavyQueen9N8:
    ...
    def init_borad(self):
        if self.file_name == None:
            ...
            extra_num = int(self.n/8)

            for i in range(extra_num):
                queen_weight, queen_pos = self.sort_queen()
                x = random.randint(0, self.n - 1)
                y = random.randint(0, self.n - 1)
                while self.__array_belong(np.array([x,y]),queen_pos):
                    x = random.randint(0, self.n - 1)
                    y = random.randint(0, self.n - 1)
                self.chess_board[x, y] = random.randint(1, 9)
                print('1')
            csv_name = f'heavyqueen9n8_{self.n}_init.csv'
            savetxt(csv_name, self.chess_board, delimiter=',')
        else:
            self.__loadboard()

    def __array_belong(self, array, list):
        flag = False
        for i in range(len(list)):
            if np.array_equal(array, list[i]):
                flag = True
        return flag
        ...
        while (True):
            ...
            for i_index in range(len(queen_weight)):
                ...
                for i_pos in range(self.n):
                    if self.check_total() == int(self.n/8):
                        print('done')
                        return
                    if self.chess_board[i_pos, col] != 0:
                        continue
                    self.chess_board[row_new, col] = 0
    ...
    def __chessboard(self):
        ...
        for i in range(side):
            for j in range(side):
                ...
                self.__square(color)
                self.marker.goto(self.greg.xcor() - self.size / 2,
                                 self.greg.ycor() + self.size / 2 - self.FONT_SIZE / 2)
...
if __name__ == "__main__":
    ...
    N = 8
    csv_name = f'heavyqueen9n8_{N}_init.csv'
    heavy_queen = HeavyQueen9N8(chess_dim=8, file_name=csv_name)
    # heavy_queen = HeavyQueen9N8(chess_dim=N)
    heavy_queen.init_borad()
    # heavy_queen.run()
    print("Runtime:  %s seconds" % (time.time() - start_time))
    c = heavy_queen.check_total()
